...
else
{
    ...
    $per_page = 10;
    $sql = "SELECT count(event_id) FROM events where delete_flag=0";
                ...
                <?php
                    echo '<table class="table">
                        ...
                        for ($i = $start; $i < $end; $i++) 
                        {
                            ...
                            // <td><img class="img_size" src="'.substr($details[$i]['event_image'], 0, strpos($details[$i]['event_image'], ',')).'"></td>
                            echo'
                                <td>'.($i + 1).'</td>
                                <td>'.$details[$i]['title'].'</td>
                                
                                <td>'.$seo_title.'</td>';      
                            echo '<td class="button-holder"><a href=add-events.php?edit_id='.$details[$i]['event_id'].'>Edit</a></td>
                                <td class="button-holder"><a class="del_class_event" href="javascript:;" id='.$details[$i]['event_id'].'>Delete</a></td>
                            </tr>';
                        }
                        echo '</tbody>
                    </table>';
